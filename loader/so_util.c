    ...
    switch (type) {
      ...
      case R_ARM_JUMP_SLOT:
      {
        if (sym->st_shndx == SHN_UNDEF) {
          ...
          if (!resolved) {
            // Unresolved: GOT/PLT slot keeps its link-time garbage; first
            // call/deref faults. NOT logged here -- so_resolve runs once per
            // table (default/audio/jni/fs/sdl/gl), so a symbol resolved by a
            // later table would false-positive on every earlier pass. Use the
            // static readelf UND-vs-provided diff for the true unresolved set
          }
        }
